...
public class RulesMenu : MonoBehaviour
{
    ...
    void Update()
    {
        if (move_down >= 0)
        {
            if (move_down >= 50 && move_down <= 150)
                Move(2, false);

            if (move_down >= 100 && move_down <= 150)
                Move(3, false);

            if (move_down >= 0 && move_down <= 150)
            {
                Move(1, false);
                Move(5, false);
                Move(6, false);
        ...
        if (move_up >= 0)
        {
            if (move_up >= 0 && move_up < 50) Move(3, true);

            if (move_up >= 0 && move_up < 100) Move(2, true);

            if (move_up >= 0 && move_up < 150)
            {
                Move(1, true);
                Move(5, true);
                Move(6, true);
    ...
    {
        rotating = true;
        Quaternion startRotation = GameObject.Find("Image1").GetComponent<Image>().transform.rotation;
        Quaternion endRotation = Quaternion.Euler(angles) * startRotation;
        for (float t = 0; t < duration; t += Time.deltaTime)
        {
            GameObject.Find("Image1").GetComponent<Image>().transform.rotation = Quaternion.Lerp(startRotation, endRotation, t / duration);
            yield return null;
        }
        GameObject.Find("Image1").GetComponent<Image>().transform.rotation = endRotation;
        rotating = false;
    }
